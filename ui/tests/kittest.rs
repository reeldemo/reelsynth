...
use reelsynth::Patch;
use reelsynth_ui::widgets::{
    adsr_graph, button_ghost, button_toggle, labeled_select, panel_audit, reel_combo,
    select_value_text, tab_bar, Knob, KnobSize, PianoKeyboard,
};
use reelsynth_ui::{
    audit_center, audit_header_clusters, audit_id_rect, audit_osc_sidebar_stacks,
    audit_panel_utilization, audit_shell, audit_theme_tokens, center_morph_used_rect_id,
    center_scope_used_rect_id, center_strip_used_rect_id, center_used_rect_id,
    center_views_used_rect_id, compute_center_regions, count_base_audit_variants,
    default_effect_slots, draw_shell, embed_piano_in_center, footer_used_rect_id,
    fx_strip_used_rect_id, header_used_rect_id, mod_strip_used_rect_id, osc_fx_allocated_rect_id,
    osc_fx_used_rect_id, osc_mod_allocated_rect_id, osc_mod_used_rect_id, osc_type_index,
    osc_used_rect_id, piano_used_rect_id, rail_filter_allocated_rect_id, rail_filter_used_rect_id,
    rail_used_rect_id, record_region, record_used, utilization, AuditId, ShellAudioDevices,
    ShellConfig, ShellLayout, ShellLayoutOptions, ShellMidiDevices, ShellMode, UiState,
    WtView3dMode, APP_HEIGHT_FULL, APP_MIN_WIDTH, APP_WIDTH, REGISTRY_VARIANT_COUNT, SPACE_SM,
};
...
fn knob_widget_harness_smoke() {
    ...
    let mut harness = Harness::builder().with_size([120.0, 120.0]).build_ui(|ui| {
...
fn compact_mode_collapses_sections() {
    ...
    let mut harness = Harness::builder().with_size([1280.0, 720.0]).build_state(
        |ctx, test| {
...
fn full_shell_min_window_no_layout_overlap() {
    ...
    let regions =
        compute_center_regions(inner, &config, scale, embed_piano_in_center(options), true);
...
fn interface_used_rects_within_allocated_min_window() {
    ...
    let morph_used = harness
        .ctx
        .data(|d| d.get_temp::<egui::Rect>(center_morph_used_rect_id()));
    assert!(
    ...
    assert!(harness
        .ctx
        .data(|d| d.get_temp::<egui::Rect>(mod_strip_used_rect_id()))
        .is_none());
    assert!(harness
        .ctx
        .data(|d| d.get_temp::<egui::Rect>(fx_strip_used_rect_id()))
        .is_none());
}
...
fn panel_whitespace_utilization_at_1280x880() {
    ...
    let mut harness = Harness::builder().with_size([1280.0, 880.0]).build_state(
        |ctx, test| {
            if !test.fonts_applied {
                reelsynth_ui_theme::apply(ctx);
                test.fonts_applied = true;
    ...
    let filter_alloc = get_used(
        &harness.ctx,
        rail_filter_allocated_rect_id(),
        "filter allocated",
    );
...
fn run_header_cluster_audit(test: ShellHarnessTest) {
    ...
    let screen = Rect::from_min_size(egui::pos2(0.0, 0.0), egui::vec2(APP_WIDTH, APP_HEIGHT_FULL));
    let layout = ShellLayout::compute_with_options(screen, options);
    audit_shell(&layout, screen, options);
...
fn design_fx_sidebar_slots() {
    ...
    scenario
        .state
        .fx_slots
        .push(reelsynth_ui::EffectSlotUi::from_slot(
            &reelsynth::fx::EffectSlot::chorus(),
        ));
    let run = run_shell_audit(scenario);
    assert_full_ui_audit(&run, &default_audit_options());
...
fn design_mod_matrix_rows() {
    ...
    while scenario.state.mod_routes.len() < 4 {
        scenario
            .state
            .mod_routes
            .push(reelsynth_ui::ModSlotUi::default());
    }
    let run = run_shell_audit(scenario);
    assert_full_ui_audit(&run, &default_audit_options());
}
...
fn design_center_scope() {
    ...
    let used = run
        .ctx
        .data(|d| d.get_temp::<Rect>(center_scope_used_rect_id()));
    assert!(used.is_some());
    assert_full_ui_audit(&run, &default_audit_options());
}
...
fn design_shape_template_maps_to_layer_type() {
    ...
    assert_eq!(
        shape_template_source_type(FrameShapeTemplate::Tri),
        "triangle"
    );
}
...
fn design_wt_3d_stack() {
    ...
    assert!(
        stack.is_some(),
        "Design right pane should record composite stack overlay"
    );
...
fn design_stack_overlay_with_layers() {
    ...
    assert!(
        stack.is_some(),
        "right pane shows composite stack overlay with layers"
    );
}
...
fn design_wt_3d_default_is_stack() {
    ...
    assert!(
        stack.is_some(),
        "default Design opens composite stack overlay"
    );
    let toggle = audit_id_rect(&run.ctx, AuditId::CenterWt3dModeToggle);
    assert!(
        toggle.is_none(),
        "Stack/Morph dual toggle not on Design home"
    );
    assert_full_ui_audit(&run, &default_audit_options());
}
...
fn design_scope_result_label() {
    ...
    let mut harness = Harness::builder().with_size([480.0, 72.0]).build_ui(|ui| {
    ...
    assert!(
        out.is_some(),
        "Result cell (audit id CenterScopeCellOut) recorded"
    );
    if let Some(r) = out {
...
fn design_wt_quant_handles() {
    ...
    assert!(
        plot.is_some(),
        "2d plot region should be recorded for quant handles"
    );
    assert_full_ui_audit(&run, &default_audit_options());
}
...
fn design_sidebar_parity() {
    ...
    assert!(
        diff < 1.5,
        "osc={} rail={}",
        run.layout.osc.width(),
        run.layout.rail.width()
    );
}
...
fn compose_arrangement_clips() {
    ...
    if scenario.state.compose.project.tracks.len() < 2 {
        scenario
            ...
            .project
            .tracks
            .push(reelsynth::Track::new("Track 2"));
    }
    let run = run_shell_audit(scenario);
    assert_full_ui_audit(&run, &default_audit_options());
}
...
fn widget_audit_registry_harness() {
    ...
    let mut harness = Harness::builder().with_size([480.0, 320.0]).build_state(
        |ctx, state| {
            if !state.applied {
                reelsynth_ui_theme::apply(ctx);
                state.applied = true;
            ...
            egui::CentralPanel::default().show(ctx, |ui| {
                panel_audit(ui, "Panel", Some(AuditId::WidgetPanel), |ui| {
                    ...
                    record_region(ui.ctx(), AuditId::WidgetTabBar, tabs_before, ui.min_rect());
