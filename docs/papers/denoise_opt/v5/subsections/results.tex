...
\begin{table*}[t]
  ...
  $R$/SNR/SDR/wrap-jump are multifamily mean$\pm$std. Columns marked * use the canonical freeze (seed $20{,}260{,}719$) for ms/batch and $\Delta R$ vs DualCosine.
...
\begin{table}[t]
  \centering
  \caption{Ablations. Left block: overnight branch-best freeze (run \texttt{gpu-rl-arch-20260719T083019Z}, final iter $\approx 6{,}922$). Right block: isolated short-budget re-runs (150 iters, seed $1{,}902{,}771{,}841$, plateau adapt off). $R$ is prolonged residual on the runner geometry.}
  \label{tab:ablation}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lrr@{}}
    \toprule
...
\begin{table}[t]
  \centering
  \caption{Compute budget for the reported overnight freeze (RTX~3090, PyTorch $2.6$+cu124, search seed $1{,}902{,}771{,}841$). Arch evaluations $\approx$ clean iterations.}
  \label{tab:compute}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lr@{}}
    \toprule
