...
\begin{table*}[t]
  ...
  $R$/SNR/SDR/wrap-jump are multifamily mean$\pm$std; ms/batch and $\Delta R$ vs DualCosine from the canonical freeze (seed $20{,}260{,}719$) when marked *.
  Primary metric $R$ (1~=~best). No PESQ/STOI.}
  \label{tab:sota-main}
...
Table~\ref{tab:ablation} freezes branch-best residuals from the overnight history at the reported gate (honest: not isolated single-branch re-runs).
GA and PBT lead among branches; the full hybrid champion is highest.
Table~\ref{tab:compute} summarizes the compute budget mined from the same run.

\begin{table}[t]
  \centering
  \caption{Minimal ablations via overnight branch-best freeze (run \texttt{gpu-rl-arch-20260719T083019Z}, final iter $\approx 6{,}922$). $R$ is prolonged residual on the runner geometry.}
  \label{tab:ablation}
  \setlength{\tabcolsep}{3pt}
  \begin{tabular}{@{}lr@{}}
    \toprule
    Config & $R$ \\
    \midrule
    Full hybrid champion & 0.99093 \\
    GA-only (branch best) & 0.99016 \\
    PBT (branch best) & 0.99012 \\
    PPO-only (branch best) & 0.98924 \\
    combo (branch best) & 0.98854 \\
    NAS (branch best) & 0.98677 \\
    DualCosine (runner) & 0.81658 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lr@{}}
    ...
    Peak VRAM (overnight) & not logged \\
    \bottomrule
  \end{tabular}\\[0.4em]
  {\footnotesize Overnight peak memory was not recorded in \texttt{history.jsonl}; bench-process peaks are not a substitute.}
\end{table}
