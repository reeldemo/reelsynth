...
Panel (b) is the cracked engine tile with open-wrap cliff ($R\approx0.931$).
Panel (c) applies DualCosine on that same tile ($R\approx0.640$ on this severe cliff).
Panel (d) is the DenoiseOpt neural favorite on the same tile ($R\approx0.986$).
The figure is the problem story in one strip: ideal, crackle, classical attempt, our aim.

\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_intro_sine_problem.png}
  \caption{Sine-wrap edge case from the canonical eval holdout (same tile). Ideal periodic reference, cracked engine with wrap seams (dashed), DualCosine bake, and DenoiseOpt favorite. Prolonged residual $R$ is printed per panel.}
