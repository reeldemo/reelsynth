...
impl VoicePool {
    ...
    pub fn note_on(
        ...
        if let Some(idx) = self
            .voices
            .iter()
            .position(|v| v.active && v.note == note && v.gate)
        {
            self.global_age += 1;
            let age = self.global_age;
            self.voices[idx].trigger(patch, note, freq, velocity, start_time);
        ...
        voice.trigger(patch, note, freq, velocity, start_time);
        self.global_age += 1;
...
mod tests {
    ...
    fn steals_when_full() {
        ...
        for n in 0..MAX_VOICES as u8 {
            pool.note_on(&patch, 60 + n, 440.0, 1.0, 0.0);
        }
        assert!(pool.voices.iter().all(|v| v.active));

        pool.note_on(&patch, 127, 880.0, 1.0, 0.0);
